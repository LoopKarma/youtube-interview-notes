...
def main(url: str, output: str, model: str, max_chunks: int | None) -> None:
    """Transcribe a YouTube video and generate a structured markdown summary."""
    load_dotenv()
    client = OpenAI()
    ...
    click.echo(f"Summarizing with {model}...")
    summary_body = summarize_transcript(transcript_text, client, model=model)
